    ...
    @term.times do |i|
      daily_ary(i).each do |day|
        ...
        save_crawling_result(url: request_url, parser: nokogiri) do |doc|
          format(date: Date.new(current_year_str.to_i,  current_month_str.to_i + i, day.to_i), doc: doc, )
        end
      end
    end
  end
  ...
  def format(date:, doc:)
    doc.css('.content-area article').each_with_object([]) do |li_element, events|
      ...
      event = OpenStruct.new(
        title: title,
        date: date,
        act: (artists_str&.split(/ /) || [])+ [artists_str], # basementbarはアーティスト名入力してないケースが多すぎる フライヤー表示も考えないとイケナイ
        info: info
      )
      events << event
    end
